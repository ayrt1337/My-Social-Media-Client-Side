...
const Reply = props => {
    ...
    return (
        <div ref={reply} onLoad={handleLoad} id={props.reply._id} style={props.index ? { marginBottom: '40px' } : { marginBottom: '0px' }} className="flex mt-9 pl-7 pr-7">
            <img onClick={(e) => {
                e.stopPropagation()
                navigate(`/profile/${props.reply.user}`)
            }} className="cursor-pointer border-[2px] border-[#660eb3] w-[55px] h-[55px] rounded-[50%]" src={props.reply.profileImg == null ? ImageProfile : props.reply.profileImg} alt="" />
            ...
            <div className="ml-3 mt-[2px] w-full">
                <div className="flex">
                    <p onClick={(e) => {
                        e.stopPropagation()
                        navigate(`/profile/${props.reply.user}`)
                    }} className="cursor-pointer mb-1 font-semibold max-w-[150px]">{props.reply.user}</p>
                ...
                {showTextarea &&
                    <div id={id} className="flex flex-col w-full mt-4 items-start">
                        ...
                        <div className="flex items-center">
                            <p onClick={() => submitReply(props.commentId)} className="text-center text-[16px] font-semibold bg-[#660eb3] mt-5 pb-2 pt-2 pl-7 pr-7 mr-4 rounded-[15px] cursor-pointer">
                                Responder
                            </p>
