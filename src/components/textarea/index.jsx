    ...
    return (
        <div id={props.id} className='flex flex-col w-full'>
            <div className='relative' ref={containerRef}>
                <Mentions onClick={handleCursor} onKeyUp={handleCursor} prefixCls='rc-mentions' getPopupContainer={() => containerRef.current} id={id} defaultValue={props.defaultValue != undefined ? props.defaultValue : null} placeholder={props.placeholder} autoSize={true} notFoundContent={content} onInput={handleInput} onBlur={handleBlur} onFocus={handleFocus} onChange={handleChange} className='post-form w-full resize-none bg-[#0f0f0f]'>
                    {result &&
                        <>
                            {
                                users.map((element, index) => {
                                    return (
                                        <Option value={element.user} onClick={() => navigate(`/profile/${element.user}`)} className="bg-[#0f0f0f] w-full flex cursor-pointer items-center text-[#ffffff]" key={index}>
                                            <img className="size-[45px] rounded-[50%]" src={element.profileImg == null ? ImageProfile : element.profileImg} alt="" />
